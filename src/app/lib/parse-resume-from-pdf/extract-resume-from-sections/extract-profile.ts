...
export const extractProfile = (sections: ResumeSectionToLines) => {
  ...
  return {
    profile: {
      name: cleanText(name),
      email: email, // Don't clean email as it contains special characters like @ and .
      phone: cleanText(phone),
      location: cleanText(location),
      url: url, // Don't clean URL as it contains special characters like / and .
      // Dedicated section takes higher precedence over profile summary
      summary: cleanText(summarySection || objectiveSection || summary),
    },
    // For debugging
    profileScores: {
      name: nameScores,
